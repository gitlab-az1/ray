...
import { convertUint8ArrayToHex } from '@@internals/utils';
import { type Either, left, right } from '@@internals/Either';
...
export class JsonWebToken<TData = any> {
  ...
  public static async decode<T>(token: string, key: Secret, salt?: string): Promise<Either<JsonWebTokenError | Exception, JsonWebToken<T>>> {
    ...
    try {
      ...
      return left(new Exception(err.message, { name: err.name, stack: err.stack, _inner: err, _innerCause: err.message }));
    }
  }
}
